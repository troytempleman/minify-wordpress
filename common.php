<?php
class MinifyWordPressCommon {

  var $p = null;

  function MinifyWordPressCommon($plugin) {
    $this->p = $plugin;
  }
  ...
  function a_clear_super_cache() {
    if ( function_exists('prune_super_cache') ) {
      prune_super_cache($this->get_plugin_dir().'/cache/', true );
    }
  }
  ...
  function get_plugin_dir() {
    $path = trailingslashit(trailingslashit(get_template_directory()));
    $path = preg_replace('/(\/themes\/[^\/]+?\/).+\//', '\1', $path);
    return $path;
  }

  function get_plugin_url() {
    $url = trailingslashit(trailingslashit(get_template_directory_uri()));
    $url = preg_replace('/(\/themes\/[^\/]+?\/).+\//', '\1', $url);
    return $url;
  }
