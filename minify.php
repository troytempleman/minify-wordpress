 ...
 *
 * Based on Minify https://github.com/mrclay/minify by Steve Clay https://github.com/mrclay
 * Successor of WP Minify plugin by Thaya Kareeson https://wordpress.org/plugins/wp-minify/ and
 * WP Minify Fix plugin by NodeCode https://wordpress.org/plugins/wp-minify-fix/.
 * Updated to Minify 3.0.3
 *
 * Unlike its plugin predecessors, this is intended to be included in your theme's inc folder.
 * Add require get_template_directory() . '/inc/minify/minify.php';  to your theme's functions.php file.
 * Go to Settings > Minify to activate and configure.
 */

class MinifyWordPress {

	var $author_homepage = 'https://github.com/troytempleman';
	var $homepage = 'https://github.com/troytempleman/minify-wordpress';
	var $name = 'minify_wordpress'; 
	var $name_dashed = 'minify-wordpress'; 
	var $name_proper = 'Minify'; 
	var $required_wp_version = '2.7';
	var $version = '1.0.0';

	var $c = null;
	var $debug = false;
	var $cache_location = TEMPLATEPATH .'/inc/minify-wordpress/cache/';
	...
	function MinifyWordPress() {
		// initialize common functions
		$this->c = new MinifyWordPressCommon($this);
	...
	function a_request_handler() {
		...
		$this->c->a_check_dir_writable($this->c->get_plugin_dir().'inc/minify/cache/', array($this, 'a_notify_cache_not_writable'));
		$this->c->a_check_orphan_options(array($this, 'a_notify_orphan_options'));
		if ($this->c->a_check_dir_writable($this->c->get_plugin_dir().'inc/minify/min/config.php', array($this, 'a_notify_config_not_writable'))) {
			$this->a_check_minify_config();
		}
	}

	function a_check_minify_config() {
		$fname = $this->c->get_plugin_dir().'inc/minify/min/config.php';
		...
		$cache_path_code = isset($matches[1]) ? $matches[1] : null;
		...
		$cache_age_code = isset($matches[1]) ? $matches[1] : null;
	...
	function a_notify_cache_not_writable() {
		$this->c->a_notify(
			sprintf('%s: %s',
				...
				$this->c->get_plugin_dir().'inc/minify/cache/'),
			true);
	}
	...
	function a_notify_config_not_writable() {
		$this->c->a_notify(
			sprintf('%s: %s',
				...
				$this->c->get_plugin_dir().'inc/minify/min/config.php'));
	}
	...
	function fetch_content($url, $type = '') {
		$cache_file = $this->c->get_plugin_dir().'inc/minify/cache/'.md5($url).$type;
		$content = '';
	...
	function build_minify_urls($locations, $type) {
		$wpm_options = get_option($this->name);
		$minify_url = $this->c->get_plugin_url().'inc/minify/min/?f=';
	...
	function get_cached_minify_urls($urls, $type) {
		...
		foreach ($urls as $url) {
			...
			$cache_url = $this->c->get_plugin_url().'inc/minify/cache/'.md5($url).$type.'?m='.filemtime($cache_file);
