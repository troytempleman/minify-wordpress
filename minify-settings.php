...
jQuery(document).ready(function($){
	$("#wpm_options_toggle_advanced").click(function(e){
		...
		if(state == "visible"){
			...
			$(this).attr("value", "<?php echo __('Show Advanced Settings', $this->name); ?>");
			$.ajax({
				type		: "POST",
				url			: "admin-ajax.php",
			...
			$(this).attr("value", "<?php echo __('Hide Advanced Settings', $this->name); ?>");
			$.ajax({
				type		: "POST",
				url			: "admin-ajax.php",
...
<?php
		...
		if ($wpm_options['show_advanced']) {
			$advanced_style = '';
			$advanced_toggle_text = __('Hide Advanced Settings', $this->name);
			$advanced_toggle_state = 'visible';
		}
		else {
			$advanced_style = 'style="display:none"';
			$advanced_toggle_text = __('Show Advanced Settings', $this->name);
		...
		printf('	
			<h2>%s</h2>	
			<p><a href="'.$this->homepage.'" target="_blank">%s</a> %s <a href="https://github.com/mrclay/minify" target="_blank">%s</a> %s <a href="https://en-ca.wordpress.org/plugins/wp-minify/" target="_blank">%s</a> %s <a href="https://en-ca.wordpress.org/plugins/wp-minify-fix/" target="_blank">%s</a> %s.</p>
			',
			__('About', $this->name),
			__('Minify', $this->name),
			__('minifies and concatenates HTML, CSS and Javascript in WordPress themes to reduce page size, load time and http requests. Based on ', $this->name),
			__('Minify', $this->name),
			__('by Steve Clay. Successor of', $this->name),
			__('WP Minify', $this->name),
			__('plugin by Thaya Kareeson and', $this->name),
			__('WP Minify Fix', $this->name),
			__('plugin by NodeCode. Updated to Minify 3.0.3', $this->name)
		);
		...
		if ( function_exists( 'wp_nonce_field' ) && wp_nonce_field( $this->name ) ) {
			printf('
				<p class="submit">
					<input type="submit" name="wpm_options_update_submit" class="button button-primary" value="%s" />
					<input type="submit" name="wpm_options_reset_submit" id="wpm_options_reset" class="button" value="%s" onclick="return confirm_reset()" '.$advanced_style.'/>
					<input type="button" id="wpm_options_toggle_advanced" class="button" state="'.$advanced_toggle_state.'" value="'.$advanced_toggle_text.'"/>
				</p>
				',
				__('Save Changes', $this->name),
				__('Default Settings', $this->name)
			);
		}
?>
</fieldset></form>
